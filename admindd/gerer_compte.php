...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if (isset($_POST['block_account'])) {
            ...
            $stmt->execute([$userId]);
            $success = "Le compte a été bloqué avec succès.";
        } 
        elseif (isset($_POST['unblock_account'])) {
            $stmt = $conn->prepare("UPDATE users SET status = 'active' WHERE id = ?");
            $stmt->execute([$userId]);
            $success = "Le compte a été débloqué avec succès.";
        }
        ...
        <nav class="col-md-3 col-lg-2 d-md-block bg-dark sidebar collapse">
            <div class="position-sticky pt-3">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active text-white" href="gerer_compte.php">
                                            ...
                                            <form method="POST" class="d-inline-block">
                                                ...
                                                <?php if ($user['role'] !== 'admin'): ?>
                                                    <?php if ($user['status'] === 'active'): ?>
                                                        <button type="submit" name="block_account" class="btn btn-warning btn-sm" 
                                                                onclick="return confirm('Êtes-vous sûr de vouloir bloquer ce compte ?')">
                                                            <i class="fas fa-ban"></i> Bloquer
                                                        </button>
                                                    <?php elseif ($user['status'] === 'blocked'): ?>
                                                        <button type="submit" name="unblock_account" class="btn btn-success btn-sm">
